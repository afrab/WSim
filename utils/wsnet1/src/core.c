...
/**************************************************************************/
/**************************************************************************/
void core_notify_carrier_sense(void)  {
  ...
  while (p_loop) {
    ...
    while (i--) {
      ...
      noise = propagation_compute_noise(&g_nodes[i], p_loop->radio, p_loop->id, g_time);
			
      /* Notify carrier sense */
      radio_carrier_sense(&g_nodes[i], p_loop->id, rx_mW, rx_mW / noise, p_loop->radio);
    }	
...
int core_start(struct _worldsens_s *worldsens) {
  ...
  while (1) {
	...
	if (worldsens_s_save_release_request(worldsens, WORLDSENS_SYNCH_PERIOD))
	  return -1;
      } 
    else if (g_time < g_events->time) 
      {
	/* If event, program event */
	if (worldsens_s_save_release_request(worldsens, g_events->time - g_time))
	  return -1;
      } 
    else 
      {
	...
	if (g_time != event->time) 
	  {
	    fprintf(stderr, "\nEXCEPTION: CORE DESYNCHRONIZATION time=%" PRId64 ", evt_time=%" PRId64 "\n",
		    g_time, event->time);
	    return -1;
	  }
	evt_nb++;
	...
	if (event->type == TX_EVENT_TYPE) 
	  {
		...
		if (worldsens_s_save_release_request_rx(worldsens, packet->node->addr,
						  ...
						  worldsens_data,  
						  31250)) // ToCheck: optimization to avoir backtracks
	    return -1;
	} else {
...
/**************************************************************************/
/**************************************************************************/
int core_update_packet_list(void) {
  struct _packet *p_loop = g_packets;
  struct _packet *p_tmp = NULL;
  struct _event *event = g_events;
  uint64_t tx_start;
