...
/**
 *  \file   wasp.c
 *  \brief  Platform devices handling functions, WASP dev nodes
...
/* ***************************************
 * platform description for WASP devices
 *
 *
 * red         @31   1 << 4      P5
 * green       @31   1 << 5      P5
...
struct wasp_struct_t {
  int flash_cs;
  int radio_cs;
};

#define SYSTEM_DATA      ((struct wasp_struct_t*)(machine.device[SYSTEM].data))
...
int system_create(int dev_num)
{
  ...
  machine.device[dev_num].state_size    = sizeof(struct wasp_struct_t);
...
int devices_create(void)
{
  ...
  xin_freq = 32768;
  xt2_freq = 0; /* xt2 is not used on wasp */

  if (xt1_opt.value) {
    xin_freq = atoi(xt1_opt.value);
    HW_DMSG_CLOCK("wasp: xt1 external crystal set to %d Hz\n",xin_freq);
  }
  ...
  machine.device_size[SYSTEM] = sizeof(struct wasp_struct_t);
...
int devices_update()
{
   ...
   */
   if (msp430_digiIO_dev_read(PORT1,&val8))
    {
      HW_DMSG("wasp: writing on port 1 byte 0x%02x at PC 0x%04x\n",val8 & 0xff,mcu_get_pc());
    }
  ...
  if (msp430_digiIO_dev_read(PORT2,&val8))
    {
      HW_DMSG("wasp: writing on port 2 byte 0x%02x at PC 0x%04x\n",val8 & 0xff,mcu_get_pc());
    }
  ...
  if (msp430_digiIO_dev_read(PORT3,&val8))
    {
      HW_DMSG("wasp: writing on port 3 byte 0x%02x at PC 0x%04x\n",val8 & 0xff,mcu_get_pc()); 
    }
  ...
  if (msp430_digiIO_dev_read(PORT4,&val8))
	...
	{
	  HW_DMSG_DEV("wasp: flash CS set to %d\n",BIT(val8,4));
	}
      if (BIT(val8,2) != SYSTEM_RADIO_CS)
	{
	  HW_DMSG_DEV("wasp: cc2420 CS set to %d\n",BIT(val8,2));
	}
#endif
  ...
  if (msp430_digiIO_dev_read(PORT6,&val8))
    {
      HW_DMSG("wasp: writing on port 6 byte 0x%02x at PC 0x%04x\n",val8 & 0xff,mcu_get_pc());
    }
  ...
  switch (MCU.usart0.mode)
      ...
      if (msp430_usart0_dev_read_spi(&val8))
	{
	  ...
	  if ((SYSTEM_FLASH_CS == 0) && (SYSTEM_RADIO_CS == 0))
	    {
	      WARNING("wasp: Flash and Radio SPI are selected at the same time\n");
	    }
  ...
  /* input on radio */
  {
	...
	if (MCU.usart0.mode != USART_MODE_SPI)
	  {
	    ERROR("wasp: read data on CC2420 radio while not in SPI mode ?\n");
	  }
	HW_DMSG("wasp: read data from CC2420 SPI = 0x%02x\n",value & CC2420_DATA_MASK);
  ...
  /* input on flash */
  {
	...
	if (MCU.usart0.mode != USART_MODE_SPI)
	  {
	    ERROR("wasp: read data on flash while not in SPI mode\n");
	  }
	if (SYSTEM_FLASH_CS != 0) /* push test above device_read ? */
	  {
	    ERROR("wasp: read data on flash without CS enabled\n");
	  }
	HW_DMSG("wasp: read data from AT45DB SPI = 0x%02x\n",value & AT45DB_D);
