...
void cc2420_assert_ccamux(struct _cc2420_t *cc2420, int event, int assert) {
    ...
    if (event == (cc2420->registers[CC2420_REG_IOCFG1] & 0x1F)) {
	    ...
	    cc2420->CCA_pin = 0xFF;
	    CC2420_DBG_MUX("cc2420_assert_ccamux: cca pin aserted 0xFF (event %x)\n", event);

	} else {
	    cc2420->CCA_pin = 0x00;
	    CC2420_DBG_MUX("cc2420_assert_ccamux: cca pin aserted 0x00 (event %x)\n", event);
	}
    }
...
void cc2420_assert_sfdmux(struct _cc2420_t *cc2420, int event, int assert) {	
    ...
    if (event == (cc2420->registers[CC2420_REG_IOCFG1] & (0x1F << 5))) {
        if (assert == CC2420_PIN_ASSERT) {
	    cc2420->SFD_pin = 0xFF;
	    CC2420_DBG_MUX("cc2420_assert_sfdmux: sfd pin aserted 0xFF (event %x)\n", event);

	} else {
	    cc2420->SFD_pin = 0x00;
	    CC2420_DBG_MUX("cc2420_assert_sfdmux: sfd pin aserted 0x00 (event %x)\n", event);
	}
    }
...
void cc2420_update_mux(struct _cc2420_t *cc2420, uint16_t val) {
    ...
    case 0x00: /* cca normal operation */
	    ...
	    CC2420_DBG_MUX("cc2420_update_mux: cca pin aserted 0xFF (event %x)\n", event);
	}
	else {
	    cc2420_assert_ccamux(cc2420, 0x00, CC2420_PIN_DEASSERT);
	    CC2420_DBG_MUX("cc2420_update_mux: cca pin aserted 0x00 (event %x)\n", event);
	}
	break;
    ...
    case 0x00: /* sfd normal operation */
	    ...
	    CC2420_DBG_MUX("cc2420_update_mux: sfd pin aserted 0xFF (event %x)\n", event);
	}
	else {
	    cc2420_assert_sfdmux(cc2420, 0x00, CC2420_PIN_DEASSERT);
	    CC2420_DBG_MUX("cc2420_update_mux: sfd pin aserted 0x00 (event %x)\n", event);
	}
	break;
    }
}
